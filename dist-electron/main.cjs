...
function startWatcher(dirPath) {
  ...
  const sendChange = (eventType, filePath) => {
    if (watcherDebounceTimer) clearTimeout(watcherDebounceTimer);
    watcherDebounceTimer = setTimeout(() => {
      mainWindow?.webContents.send("fs:external-change", { type: eventType, path: filePath });
    }, 300);
  };
  watcher.on("add", (fp) => sendChange("add", fp)).on("change", (fp) => sendChange("change", fp)).on("unlink", (fp) => sendChange("unlink", fp)).on("addDir", (fp) => sendChange("addDir", fp)).on("unlinkDir", (fp) => sendChange("unlinkDir", fp));
}
...
function scanDir(dirPath) {
  ...
  try {
    ...
    for (const item of list) {
      ...
      const isDirectory = stat4.isDirectory();
      let type = void 0;
      if (isDirectory) {
        type = "folder";
...
import_electron.ipcMain.handle("fs:startWatcher", (event, dirPath) => {
  startWatcher(dirPath);
  return true;
});
import_electron.ipcMain.handle("fs:stopWatcher", () => {
  stopWatcher();
  return true;
});
import_electron.ipcMain.handle("fs:searchFiles", async (event, vaultPath, query, options) => {
  const results = [];
...
import_electron.ipcMain.handle("fs:moveToTrash", async (event, filePath, isDirectory) => {
  try {
...
import_electron.ipcMain.handle("fs:restoreFromTrash", async (event, trashEntry) => {
  try {
...
import_electron.ipcMain.handle("fs:listTrash", async (event, vaultPath) => {
  try {
...
import_electron.ipcMain.handle("fs:emptyTrash", async (event, vaultPath) => {
  try {
    const trashDir = path.join(vaultPath, ".trash");
    if (fs.existsSync(trashDir)) {
...
import_electron.ipcMain.handle("fs:permanentDeleteTrash", async (event, trashPath) => {
  try {
    const stat4 = fs.statSync(trashPath);
    if (stat4.isDirectory()) {
...
import_electron.ipcMain.handle("fs:createDailyNote", async (event, vaultPath, date, template) => {
  try {
    ...
    let content = template.replace(/\{\{date\}\}/g, date).replace(/\{\{day\}\}/g, dayName).replace(/\{\{time\}\}/g, currentTime);
    fs.writeFileSync(filePath, content, "utf-8");
    return filePath;
  } catch (error) {
